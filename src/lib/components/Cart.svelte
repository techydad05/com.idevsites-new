<script lang="ts">
   ...
   import { addToast, confirmed } from "$lib/components/Toaster.svelte";
   export let cart: any;
   ...
   $: if (confirmed) {
         console.log("item removed", removeItemForm);
   }
                        ...
                        <div>
                           <div class="flex justify-between">
                              ...
                              <div>
                                 ...
                                 <p
                                    class="ml-4 text-sm text-gray-900 text-right"
                                 >
                                    Qty: {item.quantity}
                                 </p>
                              </div>
                        ...
                        <div class="mt-4 flex flex-1 items-end justify-between">
                           ...
                           >
                              <div class="ml-4 relative">
                                 <div class:hidden={!confirming} class="absolute right-[30px]">
                                    <h3 class="w-full">Confirm?</h3>
                                    <div class="flex justify-between">
                                       <button class="btn btn-small btn-primary">
                                          <Check />
                                       </button>
                                       <button on:click|preventDefault={(e) => confirming = false} class="btn btn-small btn-primary">
                                          <X />
                                       </button>
                                    </div>
                                 </div>
               ...
               {#if items.length > 0}
                  ...
                  <div>
                     <dl class="space-y-4">
                        <div class="flex items-center justify-between">
                           <dt class="ml-2 text-base font-medium text-gray-900">
                              Subtotal
                           </dt>
                           <dd
                              class="ml-4 mr-2 text-base font-medium text-gray-900"
                           >
                              {formatPrice(total)}
                           </dd>
                        </div>
                  ...
                  <form action="/checkout">
                     <button
                        use:close
                        type="submit"
                        class="my-4 w-full items-center justify-center rounded-md border border-transparent bg-lime-600 px-5 py-3 text-base font-medium text-white hover:bg-lime-700"
                        >Checkout</button
                     >
                  </form>
               {/if}
