...
<noscript>
  ...
  <p>
    We use a third party (<a href="https://stripe.com">Stripe</a>) to process
    credit card payments for enhanced security. Making payments on this site
...
{:else if !loading}
  <main class="lg:flex lg:min-h-full lg:flex-row-reverse lg:max-h-screen lg:overflow-hidden">
    ...
    <section
      ...
      class="flex-auto px-4 pb-16 pt-12 sm:px-6 sm:pt-16 lg:px-8 lg:pb-4 lg:pt-0 overflow-auto"
    >
      <div class="mx-auto max-w-lg">
        <!-- Logo on lg screen -->
        ...
        <form
          ...
          use:enhance={async ({ cancel }) => {
            ...
            if (!cart) {
              errorMessage =
                "Something went wrong while selecting the shipping option.";
              console.log(errorMessage);
              processing = false;
              cancel();
            }
          ...
          >
            {#each shippingOptions as shippingOption}
              <option value={shippingOption.id}
                >{shippingOption.name}
                {formatPrice(shippingOption.price_incl_tax)}</option
              >
            {/each}
          </select>
